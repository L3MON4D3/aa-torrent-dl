...
def add_torrent_paused(torrent_url) :
    client.torrents_add(urls=torrent_url, is_paused=True, category=qbt_category)
...
class QBTTorrent:
    ...
    def dl_dir(self) :
        ...
        return torrent_dir + re.match(r".*(/[^/]+)$", self.info().content_path).group(1)
...
def get_torrents(state) :
    ...
    match state:
        case TorrentState.Stopped:
            filter = lambda state_enum: state_enum.is_stopped
        case TorrentState.Downloading:
            filter = lambda state_enum: state_enum.is_downloading
        case TorrentState.Completed:
            filter = lambda state_enum: state_enum.is_complete

    assert(filter)
    return reduce(lambda list, info: list + ([QBTTorrent.from_hash(info.hash)] if filter(info.state_enum) else []), client.torrents_info(category=qbt_category), [])
...
class TorrentFile :
    def __init__(self, torrent_name, filename, targetname) :
        ...
        if re.match(r"^[a-z]_", torrent_name) != None :
    ...
    def matches(self, torrentfile) :
        fname_variations = [tname + "/" + self.fname for tname in self.torrent_name_variations]
        return torrentfile.name in fname_variations

    def store(self, torrentfile) :
        cmd = f"cp \"{torrent_dir + "/" + torrentfile.name}\" \"{target_dir +"/"+ self.targetname}\""
        os.system(cmd)
...
def exec_command(command) :
    if command["command"] == "download" :
        ...
        sendMessage(encodeMessage(f"File {target_name} is added by torrent {torrent_link} - {torrent_target_file}."))
        add_torrent_paused(torrent_link)
        watch_torrentfiles.append(TorrentFile(torrent_name, torrent_target_file, target_name))

def enable_store_torrent_files(t) :
    for f in t.files() :
        for tf in watch_torrentfiles.copy() :
            if tf.matches(f) :
                # sendMessage(encodeMessage(str(f)))
                # sendMessage(encodeMessage(str(t.info())))
                if f.progress == 1 :
                    sendMessage(encodeMessage(f"File {tf.targetname} has finished downloading."))
                    tf.store(f)
                    watch_torrentfiles.remove(tf)
                elif f.priority == priority["DoNotDownload"] :
                    sendMessage(encodeMessage(f"Starting download for file {tf.targetname}."))
                    t.set_file_prio([f], priority["Normal"])
                else :
                    sendMessage(encodeMessage(f"Progress of file {tf.targetname}: {f.progress:.2f}, progress of torrent {t.info().progress:.2f}"))
...
while True:
    ...
    for t in get_torrents(TorrentState.Downloading) + get_torrents(TorrentState.Completed) :
